...
import { identityFromPemFile, loadWasm } from "../src/ClientUtil";

async function go() {
  const host = "http://127.0.0.1:8000";
  const client = new IndexClient<IndexCanister>({
    IDL: IndexCanisterIDL,
    canisterId: "qoctq-giaaa-aaaaa-aaaea-cai", // insert your local index canister id here
    agentOptions: {
      host,
      // can choose to retrieve identity from a seed phrase file generated by quill
      /*
      identity: await identityFromSeed(
        `${homedir}/.config/dfx/identity/local-testing/seed.txt`
      ),
      */
      // or can choose to retrieve identity from a pem file generated by dfx identity new
      identity: await identityFromPemFile(
        `${homedir}/.config/dfx/identity/local-testing/identity.pem`
      ),
    },
  });
  ...
  try {
    // note - this will error if your client is not using the same identity that is a controller of the index canister
